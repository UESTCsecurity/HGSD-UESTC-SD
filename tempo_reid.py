...
# 时间边特征提取函数
def extract_temporal_edge_features(video_paths, labels, similarity_metric='cosine', seq_len=4):
    """
    提取行人视频的时间边特征
    :param video_paths: 视频文件路径列表
    :param labels: 对应的行人标签列表
    ...
    feature_extractor = TemporalFeatureExtractor().to(device)
...
def main():
    ...
    edge_features, valid_indices, valid_labels = extract_temporal_edge_features(video_paths, labels,
                                                                                similarity_metric='cosine', seq_len=4)
    ...
    # 保存时间边特征到文件
    save_temporal_edge_features(edge_features, 'temporal_edge_features.npy')

    # 加载时间边特征从文件
    loaded_edge_features = load_temporal_edge_features('temporal_edge_features.npy')
